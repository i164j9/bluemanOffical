...
from gi.repository import Gtk, Gdk, Gio, GObject  # pyright: ignore[reportMissingModuleSource]  # pylint: disable=wrong-import-position
...
DevicePropertyTypes = str | int | bool | list[str] | list[int] | Mapping[object, object] | None
...
def data_to_json(data: PropertyDataType, _values_only: bool) -> str:
...
Properties: dict[str, tuple[int, Callable[[Any], str]]] = {
    ...
    "Modalias": (170, str),
    "Adapter": (180, str),
    "ManufacturerData": (190, format_mapping),
    "ServiceData": (200, format_mapping),
    "AdvertisingFlags": (210, format_advflags),
    "AdvertisingData": (220, format_mapping),
...
class InfoWindow(Gtk.ApplicationWindow):
    def __init__(self, title: str, object_path: ObjectPath, application: Gtk.Application) -> None:
        ...
        self.__model = Gio.ListStore.new(cast(GObject.GType, getattr(InfoItem, "__gtype__")))
        ...
        self.connect("destroy", self.__cleanup_window)
    ...
    def find_item(self, description: str) -> InfoItem | None:
        return self.__items.get(description)
    ...
    def copy_to_clipboard(self) -> None:
        positions = [row.get_index() for row in self.__listbox.get_selected_rows()]
        data = self.__build_data_dict(positions)
        ...
        match self.__format:
            ...
            case _:
                logging.error("Unknown format %s, using plain", self.__format)
                data_formatter = data_to_plain_text

        text = data_formatter(data, self.__values_only)
    ...
    def __cleanup_window(self, _window: Gtk.Window) -> None:
...
class Info(ManagerPlugin, MenuItemsProvider):
    ...
    def __update_item(self, _proxy: AnyDevice, prop: str, value: object, object_path: ObjectPath) -> None:
        logging.debug("Updating %s for %s", prop, object_path)
        if object_path not in self._windows:
            return

        if prop not in Properties:
            logging.info("Skipping %s", prop)
            return

        info_window = self._windows.get(object_path)
        ...
        if item is None:
            item = self.create_item(prop, object_path, cast(DevicePropertyTypes, value))
            info_window.add_item(item)
        else:
            prop_label, value_label = self.format_property(prop, cast(DevicePropertyTypes, value))
    ...
    def _show_info(self, _menuitem: Gtk.MenuItem, device: Device) -> None:
        ...
        window = self._windows.get(object_path)
        ...
        for prop in Properties:
            ...
            try:
                ...
                logging.info("Property %s not available for device %s", prop, device['Alias'])
                continue

            if prop == "UUIDs":
                assert isinstance(value, list)
                for uuid in value:
                    item = self.create_item(prop, object_path, uuid)
                    window.add_item(item)
            else:
                item = self.create_item(prop, object_path, value)
                window.add_item(item)

        window.present()

    def __on_window_delete(self, window: InfoWindow, _event: Gdk.Event) -> bool:
        logging.debug(window.object_path)
        del self._windows[window.object_path]
        return False
